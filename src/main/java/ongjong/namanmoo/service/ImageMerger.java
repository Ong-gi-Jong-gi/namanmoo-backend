...
public class ImageMerger {
    ...
    public static BufferedImage mergeImages(List<BufferedImage> images) throws IOException {
        ...
        int finalWidth = size == 1 ? width + 2 * padding : (width * 2) + 3 * padding;
        int finalHeight = size <= 2 ? height + 2 * padding : (height * 2) + 3 * padding;
        ...
        for (int i = 0; i < size; i++) {
            ...
            int x = padding + (i % 2) * (width + padding) + (width - imgWidth) / 2;
            int y = padding + (i / 2) * (height + padding) + (height - imgHeight) / 2;
