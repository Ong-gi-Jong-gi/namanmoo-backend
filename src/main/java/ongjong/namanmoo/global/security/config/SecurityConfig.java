...
import ongjong.namanmoo.global.security.handler.CustomLogoutHandler;
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(authorize -> authorize
                        .requestMatchers("/signup", "/", "/signup/duplicate", "/login","/api/refresh-token").permitAll() // signup, login 페이지는 인증 없이 접근 가능하도록 설정
...
//                        .logoutSuccessUrl("https://mooluck.site/") // 로그아웃 성공 시 리다이렉트될 URL 설정
                        ...
                        .addLogoutHandler(customLogoutHandler()) // 쿠키 삭제
                        .logoutSuccessHandler(logoutSuccessHandler()))// 로그아웃 성공 시 처리할 핸들러
    ...
    public LogoutHandler customLogoutHandler() {
        return new CustomLogoutHandler(jwtService); // JwtService 주입
    }
    ...
    public LogoutSuccessHandler logoutSuccessHandler() {
        return (request, response, authentication) -> {
            ...
            // 직접 JSON 문자열 작성
            String jsonResponse = "{\"status\": \"200\", \"message\": \"Logout success\"}";
